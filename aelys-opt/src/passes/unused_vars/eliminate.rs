...
pub fn eliminate_unused_in_block(stmts: &mut Vec<TypedStmt>, used_vars: &HashSet<String>, stats: &mut OptimizationStats) {
    for stmt in stmts.iter_mut() {
        eliminate_unused_in_stmt(stmt, used_vars, stats);
    }

    stmts.retain(|stmt| {
        if let TypedStmtKind::Let { name, initializer, is_pub, .. } = &stmt.kind {
            if *is_pub { return true; } // keep pub exports
...
fn eliminate_unused_in_stmt(stmt: &mut TypedStmt, used_vars: &HashSet<String>, stats: &mut OptimizationStats) {
    match &mut stmt.kind {
        ...
        TypedStmtKind::If { then_branch, else_branch, .. } => {
            ...
            if let Some(else_b) = else_branch { eliminate_unused_in_stmt(else_b, used_vars, stats); }
        }
...
fn eliminate_unused_in_function(func: &mut TypedFunction, used_vars: &HashSet<String>, stats: &mut OptimizationStats) {
    let mut local_used = used_vars.clone();
    for stmt in &func.body { super::analysis::collect_uses_in_stmt_recursive(stmt, &mut local_used); }
    for param in &func.params { local_used.insert(param.name.clone()); }
...
fn has_side_effects(expr: &TypedExpr) -> bool {
    match &expr.kind {
        ...
        TypedExprKind::Binary { left, right, .. } => has_side_effects(left) || has_side_effects(right),
        ...
        TypedExprKind::If { condition, then_branch, else_branch } => {
            has_side_effects(condition) || has_side_effects(then_branch) || has_side_effects(else_branch)
        }
